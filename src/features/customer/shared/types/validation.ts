...
export function isCoupon(data: unknown): data is Coupon {
  ...
  return (
    ...
    (obj.maxRedemptions === undefined || obj.maxRedemptions === null || typeof obj.maxRedemptions === 'number') &&
...
export function isVoucher(data: unknown): data is Voucher {
  ...
  return (
    ...
    obj.expiryDate instanceof Date &&
    (obj.usedAt === undefined || obj.usedAt === null || obj.usedAt instanceof Date) &&
    typeof obj.qrCode === 'string' &&
    (obj.paymentId === undefined || obj.paymentId === null || typeof obj.paymentId === 'string') &&
    typeof obj.usageInstructions === 'string' &&
...
export function validateCouponData(data: unknown): SchemaValidationResult<Coupon> {
  ...
  if (obj.maxRedemptions !== undefined && obj.maxRedemptions !== null && (typeof obj.maxRedemptions !== 'number' || obj.maxRedemptions <= 0)) {
...
export function validateVoucherData(data: unknown): SchemaValidationResult<Voucher> {
  ...
  if (obj.usedAt !== undefined && obj.usedAt !== null && (!(obj.usedAt instanceof Date) || isNaN(obj.usedAt.getTime()))) {
    errors.push({ field: 'usedAt', message: 'Used date must be a valid Date object, null, or undefined', code: 'INVALID_DATE' });
  }
...
export function validatePaymentData(data: unknown): SchemaValidationResult<CouponPaymentData> {
  ...
  // Required string fields
  const requiredStringFields = ['couponId', 'userId'];
  for (const field of requiredStringFields) {
    if (!obj[field] || typeof obj[field] !== 'string' || (obj[field] as string).trim() === '') {
      errors.push({ field, message: `${field} is required and must be a non-empty string`, code: 'REQUIRED_FIELD' });
    }
  ...
  // Currency validation with format check
  if (!obj.currency || typeof obj.currency !== 'string' || !validateCurrencyCode(obj.currency as string)) {
    errors.push({ field: 'currency', message: 'Currency must be a valid 3-letter ISO code', code: 'INVALID_CURRENCY' });
  }
