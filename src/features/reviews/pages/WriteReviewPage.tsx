...
import { getRatingText, validateReviewForm, formatCharacterCount } from '../utils/reviewUtils';
...
const WriteReviewPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateReviewForm(restaurantName, rating, reviewText)) {
      return;
    }
  ...
  const isFormValid = validateReviewForm(restaurantName, rating, reviewText);
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm">
          ...
          <div className="flex justify-between items-center mt-2">
            <span className="text-xs text-gray-500">
              {formatCharacterCount(reviewText.length, 500)}
            </span>
          </div>
        </div>
